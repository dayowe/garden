...
unsigned long previousMillis = 0; // Timing variable for sensor data update
unsigned long lastSensorSwitchTime = 0; // Timing variable for sensor switching
...
void detectSensors() {
    ...
    for (int sensorID = 1; sensorID <= maxSensors; sensorID++) {
        ...
        delay(20); // Short delay for stable RS485 communication
    }
...
// Declare the readAndPublishSensorData function before setup
...
void setup() {
    ...
    // If no sensors are active, handle this scenario
    if (displaySensorID == -1) {
        AtomS3.Display.println("No sensors detected.");
    } else {
        // If sensors are detected, display the sensor data immediately
...
void loop() {
    ...
    // Handle 1-minute interval for updating sensor data
    if (currentMillis - previousMillis >= interval) {
        previousMillis = currentMillis;
        // Read and publish data for the current sensor
        readAndPublishSensorData(displaySensorID);
    }

    // Handle 4-second interval for switching sensors
